  	...
  	<div class="content">
		...
		<div class="main">
			<div class="enr_input all">
				<form>
					<ul>
						<li class="line">
							<div class="einput">
								<span>手机号</span>
								<input type="text" name="" value="" v-model="dataJson.params">
							</div>
						</li>
						<li class="line">
							<div class="einput">
				...
				<a href="javascript:;" class="register_btn" @click="commitFun">注册</a>
			</div>
		</div>
  	</div>
</template>
...
export default {
  ...
  data () {
    return {
      dataJson : {type:this.$route.query.type,openid:this.$route.query.openid},
      noget : 1,
      settimes : 60
    }
  },  
  methods: {
    commitFun : function(){
    	this.$http.post('/Shop/Login/theThirdRegister', this.dataJson,{emulateJSON:true}).then(function(response){	
    ...
    getCode : function () { //获取短信验证码
        this.$http.post('/Shop/Login/getPhoneOrEmailVerify', {phone_or_email:this.dataJson.params,type:4},{emulateJSON:true}).then(function(response){	
			if( response.data.status == '200000' ){
