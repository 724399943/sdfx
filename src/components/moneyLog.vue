<template>
  	<div class="content" id="JmoneyLog">
  		<header class="head">
		...
		<div class="main">
			<div class="recordWrap">
                ...
                <span class="fail" v-else-if="item.type == 2">-{{item.amount}}</span>
							</div>
						</li>						
					</ul>
				</div>
...
export default {
  ...
  methods: {
    getData(){
      if( this.hasAjax == 0 ){
        this.hasAjax = 1;
        this.$http.post('/Shop/Testing/paymentList', {page:this.page},{emulateJSON:true}).then(function(response){ 
          this.hasAjax = 0;
          if( this.page == 0 ){
    ...
    loadMore(){         
        var that = this;
        this.$store.commit('scrollFun',{dom:'JmoneyLog',auto:true,bottomCall:function(){
          that.getData();
        }})
    },
    getSign : function(){
