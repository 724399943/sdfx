			...
			<div class="goods_detail_wrap">
				<div class="ggoodsImg_wrap">
					<header class="head gdhead">
						<a href="javascript:;" @click="routeBack"><span class="back white"></span></a>
						    	...
						    	<swiper-item class="black" >
						    			<div class="goods_img_cont">
											<div class="goods_img_box">												
												<img class="Sw-img" :src="item.goods_image" v-if="!item.goods_image_ori ">
												<img class="Sw-img" :src="item.goods_image_ori" v-else>
											</div>
										</div>
						    	</swiper-item>
						    </template>
...
export default {
  ...
  methods: {
    ...
    shareFun : function(type){
    	var name = encodeURIComponent(this.detail.goods_name);
    	var url = encodeURIComponent(this.webSite+'/goodsDetail?id='+this.$route.query.id);
    	var imgurl = this.webSite+this.detail.goods_image;
    	window.location.href = 'mitchell://'+type+'?text='+name+'&url='+url+'&image='+imgurl;
    },
    routeBack : function(){
      this.$router.go(-1);
    }
