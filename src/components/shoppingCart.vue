									...
									<a href="javascript:;">
										<li>
											<div class="s_goods_wrap">
												<div class="s_cbox">
													<input type="checkbox" name="" :id="item.id" :value="item.id" v-model="shop_id" @change="checkFun(item.goods_id,item.goods_number,item.goods_price)"> 
...
export default {
  ...
  methods: {
  	...
  	checkFun : function(goods_id,goods_number,goods_price){
  		var el = document.getElementById('AllCheck');  		
  		if( event.target.checked ){
  			this.goodsID.push(goods_id);
  			var money = parseInt(goods_number) * parseInt(goods_price);
  			this.payables.push(money);
  		}else{
  		...
  		if( this.payables.length > 0 ){
  			this.total = this.payables.reduce(this.getSum);  			
  		}else{
  			this.total = 0;
  		}
  	},
  	checkedAll : function(){
  		...
  		if( event.target.checked ){
	  		for(var i = 0;i < this.list.length;i++){
	  			...
	  			var num = parseInt(this.list[i]['goods_number']) * parseInt(this.list[i]['goods_price']);
	  			this.payables.push(num);
	  		}
  			this.total = this.payables.reduce(this.getSum);
  		}else{
  			this.total = 0;
